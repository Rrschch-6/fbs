...
# Drop rows with N in ground truth
df = df[df["CODE"] != "N"]

# Split few-shot examples and evaluation data
few_shot_examples = df.head(few_shot_k)
eval_df = df.iloc[few_shot_k:]  # exclude few-shot from evaluation

texts = eval_df["UTTERANCE"].astype(str).tolist()
labels = eval_df["CODE"].astype(str).tolist()
...
FEW_SHOT_BLOCK = "\n".join(
    [f'Text: "{row["UTTERANCE"]}" → Label: {row["CODE"]}' for _, row in few_shot_examples.iterrows()]
)
...
print(f"\n✓ Done. Results + responses saved in: {root_file}/results and {root_file}/responses")
