...
os.makedirs(f"{root_file}/responses", exist_ok=True)   # 🔹 new folder for responses
...
# Candidate labels
CANDIDATE_LABELS = ["F", "Be", "Bs", "S", "D", "N"]
LABEL_PATTERN = re.compile(r'(?<![A-Za-z])(F|Be|Bs|S|D|N)(?![A-Za-z])')
...
SYN_TO_LABEL = {
    ...
    "structure": "S",
    "none": "N", "uncertain": "N", "not sure": "N", "unknown": "N",
...
Allowed labels: F, Be, Bs, S, D, N.

- Always prefer F, Be, Bs, S, or D if the text matches in any way.
- Use 'N' only if the segment clearly does not fit any of the categories at all (e.g., irrelevant, vague opinion, or off-topic).
- Do NOT choose 'N' simply because the case is difficult or ambiguous — instead select the closest matching category.
...
Note: The label set also includes 'D' (present in the data). If no category clearly applies, prefer 'N'.

Instructions: Choose exactly ONE label from [F, Be, Bs, S, D, N].
Return your answer as JSON: {"label": "<F|Be|Bs|S|D|N>"} and nothing else.
"""
...
def parse_label(generated: str) -> str:
    ...
    # "Label:" lines
    m = re.search(r'(?:Final\s*Label|Label)\s*[:\-]\s*(F|Be|Bs|S|D|N)\b', text, flags=re.IGNORECASE)
    if m:
        return m.group(1)
